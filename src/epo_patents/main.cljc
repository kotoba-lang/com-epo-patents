(ns epo-patents.main
  "Kotodama WASM entrypoint for the epo_patents clean-room actor (L4) — Clojure port.
  ...
  facts (`epo_patents.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "epo_patents")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"epo_patents.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "epo_patents-compat" :tier tier :entities entities} 200])
...
(defn start! [] :epo_patents-compat/ready)
